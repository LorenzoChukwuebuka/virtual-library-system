...
<div>
		...
		<div class="col s12">
			<ul class="tabs">
				<li class="tab col s3"><a class="active" href="#upload">upload files</a></li>
				<li class="tab col s3"><a href="#article">Articles and Journals </a></li>
		   ...
		   <div id="upload" class="col s12">
			   <br/>
			    <!-- Modal Trigger -->
	           <a class="waves-effect waves-light blue darken-2 btn modal-trigger sht" href="#modal2"> Upload <i class="material-icons"> add </i>  </a>
			...
			<tr>
		     ...
		     <div id="article" class="col s12">
				 <br/>
				 <!-- modal trigger -->
				  <a class="waves-effect waves-light blue darken-2 btn modal-trigger sht" href="#modal3"> articles &amp; Journals </a>
	   ...
	   <table class="container responsive-table highlight">
		 ...
		 <tr>
			...
			<td> {{art.post}} </td>
			<td> {{art.category}} </td>
			<td> <button class="btn waves-effect red waves-light" @click="delArticle(art.Aid)"><i class="material-icons"> delete </i> </button></td>
			<td> <a class="btn waves-effect green waves-light modal-trigger"  href="#modal4" @click.prevent="beginEdit(art)"><i class="material-icons"> edit </i> </a></td>
		</tr>
		     ...
		     <div id="archive" class="col s12">
				 <br/>
				  ...
				  <!-- modal trigger -->
				  <a class="waves-effect waves-light blue darken-2 btn modal-trigger sht" href="#modal5"> Archive </a>
		 ...
		 <tr>
			...
			<td> {{arc.file}} </td>
			<td> {{arc.type}} </td>
			<td> <button class="btn waves-effect red waves-light" @click="delArchive(arc.Id)"><i class="material-icons"> delete </i> </button></td>
		...
		<div class="modal-content">
			<h4>Add new Journals/article  </h4>
			  ...
			  <div class="row col s12">
				<div class="input-field col s10">
			      <i class="material-icons">store</i>
				<input id="email" type="text" placeholder="title" v-model="article.title" class="validate"  required>
                    ...
                    <div class="input-field col s12">
						<textarea class="materialize-textarea" placeholder="write article" v-model="article.article" data-length="10000" required></textarea>
				  ...
				  <div class='row '>
					 <div class="input-field col s6">
					<button class="btn waves-effect blue waves-light"  @click="submitArticle"> Add <i class="material-icons right">send</i> </button>
					 </div>
				</div>
		...
		<div class="modal-content">
			<h4> Edit Journals/article  </h4>
			  ...
			  <div class="row col s12">
				<div class="input-field col s10">
			      <i class="material-icons">store</i>
				<input id="email" type="text" placeholder="title" v-model="edit.title" class="validate"  required>
                    ...
                    <div class="input-field col s12">
						<textarea class="materialize-textarea" placeholder="write article" v-model="edit.article" data-length="10000" required></textarea>
					 </div>
				  ...
				  <div class='row '>
					 <div class="input-field col s6">
					<button class="btn waves-effect blue waves-light"  @click="editArticle"> submit <i class="material-icons right">send</i> </button>
					 </div>
				</div>
	   ...
	   <div id="modal5" class="modal">
		<div class="modal-content">
			<h4>Add new file </h4>
				...
				<div class="input-field col s10">
							 <i class="material-icons">store</i>
				<input id="email" type="text" placeholder="title" v-model="archive.title" class="validate"  required>
			 
				 </div>

							<div class="input-field col s12">
						 	<i class="material-icons">people</i>
						<select class="browser-default" v-model="archive.category" required>
							<option value="" disabled selected> Choose category  </option>
							<option v-for="cats in category" :key="cats.Id" v-bind:value="cats.Id" >{{cats.category}} </option>
						</select>
                    ...
                    <div class="input-field col s12">
						<textarea class="materialize-textarea" v-model="archive.post" placeholder="write article"  data-length="10000" required></textarea>
					 </div>
										...
										<div class="btn blue darken-2">
										<span>Pick File</span>
										<input  type="file" ref="file"  v-on:change="handleUpload()"  name="fileToUpload" >
									<br>
									...
									<div class='row '>
					 <div class="input-field col s6">
					<button class="btn waves-effect blue waves-light" @click="postArchive"> Add <i class="material-icons right">send</i> </button>
					 </div>
 ...
 methods:{
		...
		submitArticle(e){
			e.preventDefault()
			 ...
			 axios.post('http://localhost:5000/api/admin/articles',this.article)
			 .then(res=>{
		...
		delArticle(Aid){
		  	axios.delete(`http://localhost:5000/api/admin/articles/${Aid}`)
			.then(res=>{
			  M.toast({html:"File deleted successfully"})
			  	setTimeout(() => {
				 location.reload()
		...
		editArticle(e){
		   ...
		   axios.put(`http://localhost:5000/api/admin/articles/${Id}`,this.edit)
			.then(res=>{
				if(res.data === 200){
		...
		getArchive(){
			axios.get('http://localhost:5000/api/admin/archive')
			.then(res=>{
		...
		postArchive(e){
			...
			axios.post(`http://localhost:5000/api/admin/archive`,data,
			{
				 onUploadProgress:function(progressEvent){
		...
		delArchive(Id){
           axios.delete(`http://localhost:5000/api/admin/archive/${Id}`)
		   .then(res=>{
			   if(res.data == 200){
